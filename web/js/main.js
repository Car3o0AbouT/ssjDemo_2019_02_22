layui.use(['element','layer'],function(){
    ...
    $('.flexible').on('click',function(){
        ...
        if(objHeader.css('margin-left')=='60px'){
            objHeader.animate({'marginLeft':'220px'});
            objBody.animate({'marginLeft':'220px'});
            objSide.animate({'width':'220px'});
            objIcon.removeClass('layui-icon-spread-left');
            objIcon.addClass('layui-icon-shrink-right');
        } else {
            objHeader.animate({'marginLeft':'60px'});
    ...
    $('#personalEdit').on('click',function(){
        layer.open({
            ...
            anim: 0,
            shade: [0.5, 'rgb(0,0,0)'],
            content: '/page/personal/personalEdit',
    ...
    $('#personalPassword').on('click',function(){
        layer.open({
            ...
            anim: 0,
            shade: [0.5, 'rgb(0,0,0)'],
            content: '/page/personal/personalPassword',
            zIndex: layer.zIndex, //重点1
            success: function(layero){
                //layer.setTop(layero); //顶置窗口
            },
