...
import com.yx.demo.util.ApplicationConfig;
import com.yx.demo.util.Constants;
import com.yx.demo.util.RspVo;
...
public class MainController {
	...
	@ResponseBody
	public RspVo noLogin() {
		RspVo vo = new RspVo();
		vo.setErrorMsg("用户未登录");
		return vo;
	}

	/**
	 * 单纯的页面跳转,需在WEB-INF/page/下
	 * @param name 页面名称，即jsp文件名
	 * @return
	 */
	 ...
	 * @return
	 */
	@RequestMapping(value = "/page/{model}/{submodel}/{fun}", method = RequestMethod.GET)
	public String page(@PathVariable String model, @PathVariable String submodel, @PathVariable String fun) {
		return model + "/" + submodel + "/" + fun;
	}
	...
	public Map<String, Object> upload(MultipartFile file) {
		...
		fileName = System.currentTimeMillis() + fileName.substring(lastIndex);
		try {
			if (uploadFile(file.getBytes(), cfg.getLocalPath() + fileName)) {
				map.put("code", "0");
				map.put("msg", file.getOriginalFilename()+",上传成功");
				Map<String,String> data = new HashMap<>(1);
				data.put("src",cfg.getNetPath() + fileName);
				map.put("data",data);
			} else {
				map.put("code", "1");
			}
	...
	public void check(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		req.getSession().setAttribute(Constants.SESSION_CODE, random);
	...
	private String drawRandomNum(Graphics2D g, String... createTypeFlag) {
		...
		String baseNumLetter = "0123456789ABCDEFGHJKLMNPQRSTUVWXYZ";
	...
	private String createRandomChar(Graphics2D g, String baseChar) {
		...
		for (int i = 0; i < 4; i++) {
			...
			sb.append(ch);
			g.rotate(degree * Math.PI / 150, x, 20);
	 ...
	 */
	public static boolean uploadFile(byte[] fileBytes, String filePath) {
		boolean isOk = false;
